...
#endif
{
	...
	else if ((unsigned char)(data)[0] == (unsigned char)ID_CONNECTION_COOKIE && length <= sizeof(unsigned char)* 3)
	{
		set.cookie = *(WORD *)(data + 1);
	}
...
//	unsigned numberOfBytesUsed;
	...
	while (rcs)
	{
		...
		if (rcs->nextRequestTime < timeMS)
		{
			...
			*(WORD*)&c[1] = set.cookie ^ NETCODE_OPENCONNLULZ;
			set.cookie = 0x0000;
