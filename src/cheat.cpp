...
static void cheat_main_actor(double time_diff)
{
	...
	if (cheat_state->_generic.sync)
		cheat_state->_generic.sync = 0;
	if (cheat_state->_generic.afk &&
		vect3_length(actor_info_get(-1, 0)->speed) != 0.0f)
	...
	cheat_handle_unfreeze(NULL, info);
	...
	if (info->pedFlags.bInVehicle)
		cheat_handle_vehicle_fast_exit(NULL);
...
static void cheat_main_vehicle(double time_diff)
{
	...
	cheat_handle_unfreeze(info, NULL);
	...
	cheat_handle_vehicle_fast_exit(info);
