...
static void ini_init(void)
{
	...
	// set start settings
	set.shporaoffset = pPresentParam.BackBufferHeight / 4;
	set.piz_speed = 1.5f;
	set.cpm_offset = -5.0f;
	set.coord_item = 255;
	set.damagtype = 0;
	set.mex_price = 100;
	set.flood_delay = 1000;
	...
	if (fp)
	{
		...
		fclose(fp);
		if (strstr(dest, "#offad"))
			g_bShowAd = 0;
	}*/


	// admin checker
	sprintf(path, "%s\\" M0D_FOLDER "admins.ini", g_szWorkingDirectory);
	fp = fopen(path, "r");
	if (fp)
	{
		for (int i = 0; i < 300; i++)
	...
	// friend checker
	sprintf(path, "%s\\" M0D_FOLDER "friends.ini", g_szWorkingDirectory);
	fp = fopen(path, "r");
	if (fp)
	{
		for (int i = 0; i < 300; i++)
	...
	if ( (ent = ini_register_entry("fakestat_item", TYPE_STRING)) != NULL )
	{ 
		for ( i = 0; i < 22; i++ )
			ini_register_data( ent, set.fakestat + i, "" ); 
	}

	if ( (ent = ini_register_entry("carshot_speed", TYPE_FLOAT)) != NULL )
		ini_register_data( ent, &set.carshot_speed, "130.0f" );

	if ( (ent = ini_register_entry("pick_dist", TYPE_FLOAT)) != NULL )
		ini_register_data( ent, &set.pick_dist, "150.0f" );

	if ( (ent = ini_register_entry("search_pick", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.search_pick, "1" );

	if ((ent = ini_register_entry("info_bar", TYPE_BOOL)) != NULL)
		ini_register_data(ent, &set.show_infobar, "true");

	if ((ent = ini_register_entry( "car_find", TYPE_INT)) != NULL)
		ini_register_data(ent, &set.find_id, "462");

	if ( (ent = ini_register_entry("narko_gramm", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.narkokolvo, "1" );

	if ( (ent = ini_register_entry("admin_checker", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.adminchecker, "1" );

	if ((ent = ini_register_entry("admin_warning", TYPE_INT)) != NULL)
		ini_register_data(ent, &set.adminsound, "1");

	if ((ent = ini_register_entry("friend_checker", TYPE_INT)) != NULL)
		ini_register_data(ent, &set.friendchecker, "1");

	if ((ent = ini_register_entry("show_connects", TYPE_INT)) != NULL)
		ini_register_data(ent, &set.show_connecteds, "1");

	if ( (ent = ini_register_entry("narko_time", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.narkotime, "600" );

	if ( (ent = ini_register_entry("narko_health", TYPE_FLOAT)) != NULL )
		ini_register_data( ent, &set.narkohits, "50" );

	if ( (ent = ini_register_entry("gruz_time", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.gruz_time, "10" );

	if ((ent = ini_register_entry("meshok_count", TYPE_INT)) != NULL)
		ini_register_data(ent, &set.meshok_count, "50");

	if ((ent = ini_register_entry("gruz_wait", TYPE_INT)) != NULL)
		ini_register_data(ent, &set.gruz_wait, "1500");

	if ( (ent = ini_register_entry("afk_time", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.time_afk, "600" );

	if ( (ent = ini_register_entry("coord_distance", TYPE_FLOAT)) != NULL )
		ini_register_data( ent, &set.coord_dist, "100.0f" );

	if ((ent = ini_register_entry("coord_height", TYPE_FLOAT)) != NULL)
		ini_register_data(ent, &set.coord_height, "-50.0f");

	if ( (ent = ini_register_entry("coord_time", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.coord_time, "1500" );
	...
	if ( (ent = ini_register_entry("antisat_on", TYPE_INT)) != NULL )
		ini_register_data( ent, &set.sat, "1" );

	// secondary key
	if ( (ent = ini_register_entry("secondary_key", TYPE_KEY)) != NULL )
		ini_register_data( ent, &set.secondary_key, "lalt" );

	if ( (ent = ini_register_entry("dialog_hide", TYPE_KEY)) != NULL )
		ini_register_data( ent, &set.key_dhide, "d" );
	if ( (ent = ini_register_entry("key_coord", TYPE_KEY)) != NULL )
		ini_register_data( ent, &set.key_coord, "c" );
	if ( (ent = ini_register_entry("key_inviz", TYPE_KEY)) != NULL )
		ini_register_data( ent, &set.key_inviz, "i" );
	...
	if ( (ent = ini_register_entry("recording_activated", TYPE_BOOL)) != NULL )
		ini_register_data( ent, &set.recording_activated, "true" );
	...
	*/
	...
	/* coord teleport */
	if ( (ent = ini_register_entry("coord_teleport_name", TYPE_STRING)) != NULL )
	{
		for ( i = 0; i < 200; i++ )
			ini_register_data( ent, set.coord_teleport_name + i, "" );
	}

	if ( (ent = ini_register_entry("coord_teleport_posx", TYPE_FLOAT)) != NULL )
	{
		for ( i = 0; i < 200; i++ )
			ini_register_data( ent, set.coord_posx + i, "0.0" );
	}

	if ( (ent = ini_register_entry("coord_teleport_posy", TYPE_FLOAT)) != NULL )
	{
		for ( i = 0; i < 200; i++ )
			ini_register_data( ent, set.coord_posy + i, "0.0" );
	}

	if ( (ent = ini_register_entry("coord_teleport_posz", TYPE_FLOAT)) != NULL )
	{
		for ( i = 0; i < 200; i++ )
			ini_register_data( ent, set.coord_posz + i, "0.0" );
	}
	...
	if ( (ent = ini_register_entry("key_weapon", TYPE_KEY)) != NULL )
		ini_register_data( ent, &set.key_weapon, "f4" );
	...
	// hud indicators
	if ((ent = ini_register_entry("hud_indicator_dialog", TYPE_BOOL)) != NULL)
		ini_register_data(ent, &set.hud_indicator_dialog, "true");
	...
	if ( (ent = ini_register_entry("hud_indicator_newcheats", TYPE_BOOL)) != NULL )
		ini_register_data( ent, &set.new_cheats_indicators, "true" );

	if ( (ent = ini_register_entry("hud_indicator_batstat", TYPE_BOOL)) != NULL )
		ini_register_data( ent, &set.hud_indicator_batstat, "false" );

	if ( (ent = ini_register_entry("hud_indicator_time", TYPE_BOOL)) != NULL )
		ini_register_data( ent, &set.hud_indicator_time, "false" );
