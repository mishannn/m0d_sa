...
bool vehicleJumper ( int iVehicleID )
{
	...
	if ( g_SAMP != NULL )
	{
		...
		if (pVehicle->passengers[0] == NULL) 
			g_RakClient->SendEnterPlayerVehicle(iVehicleSAMPID, 0);
...
{ 
	...
	if ( KEY_DOWN(set.key_unflip) || cheat_state->_generic.carshot && cheat_state->state == CHEAT_STATE_VEHICLE ) 
	{ 
...
void cheat_handle_vehicle_fast_exit ( struct vehicle_info *vehicle_info )
{
