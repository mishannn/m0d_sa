...
*/
#define INI_STRLEN_MAX		512	/* including null */
...
struct settings
{
	//char					log_line[64][256];

	bool					show_infobar;
	unsigned short			cookie;
	int						mex_price;
	int						flood_delay;
	float					coord_target[3];
	char					fakerejoinpass[128];
	bool					spawnsethp;
	...
	char					coord_teleport_name[200][64];
	float					coord_posx[200];
	float					coord_posy[200];
	float					coord_posz[200];
	int						coord_item;
	int						hb_color;
	...
	int						new_cheats_indicators;
	int						hud_indicator_batstat;
	int						hud_indicator_time;
	int						hud_fps_draw;
	...
	*/
	...
	int						key_rejoin;
	int						rejoin_delay;
	int						autorejoin_delay;
	char					autorejoin_pass[128];
	int						send_spec_data;
	...
	stNetPatch				*netPatchAssoc[256][4];
