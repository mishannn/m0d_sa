...
const char *gta_vehicle_class_name ( int id )
{
	...
	case VEHICLE_CLASS_CAR:
		return "Обычные машины";

	case VEHICLE_CLASS_CAR_FAST:
		return "Спортивные машины";

	case VEHICLE_CLASS_HEAVY:
		return "Тяжелые машины";
	...
	case VEHICLE_CLASS_BIKE:
		return "Велосипеды/Мотоциклы";

	case VEHICLE_CLASS_BOAT:
		return "Лодки/Катера";

	case VEHICLE_CLASS_MINI:
		return "Легкие машины";
...
int screen_actor_find_nearest()
{
	...
	float aimpos[2] = { pPresentParam.BackBufferWidth / 2, pPresentParam.BackBufferHeight / 2 };
	...
	for (n = 0; n < pool_actor->size; n++)
	{
		if ((info = actor_info_get(n, 0x01)) == NULL)
			continue;

		if (self->base.matrix == info->base.matrix)
			continue;

		pos.x = info->base.matrix[12];
		pos.y = info->base.matrix[13];
		pos.z = info->base.matrix[14];
		CalcScreenCoors(&pos, &screenpos);
		float out[2], playerpos[2] = { screenpos.x, screenpos.y };

		vect2_vect2_sub(aimpos, playerpos, out);

		if (dist < 0.0f || vect2_length(out) < dist)
		{
			dist = vect2_length(out);
			id_nearest = getSAMPPlayerIDFromGTAPed(info);
		}
	}
