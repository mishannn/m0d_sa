	...
	if ( !g_renderSAMP_initSAMPstructs )
	{
		...
		*(BYTE*)0xBAB318 = 0;  *(BYTE*)0x53E94C = 0;
		...
		/*if (g_bShowAd)
		{
			...
			addMessageToChatWindow("|    {FFFFFF}После посещения сайта данное окно больше не будет показываться{FFFF00}     |");
			addMessageToChatWindow("=======================[Stealer by Karabawka]=====================");
			memset_safe((void *)(g_dwSAMP_Addr + 0xB3960), 0xC3, 1);
			showSampDialog(0, 1488, 0, "Stealer by Karabawka", "{FF0000}>>> Желаешь вырасти из простого читера в настоящего взломщика SA:MP аккаунтов? <<<\n>>> Хочешь подзаработать реальных денег, лишь применив смекалку и фантазию? <<<\n>>> Хочешь познать SA:MP рынок, продавая аккаунты/вирты, добытые с нуля? <<<\n{FFFF00} - Узнай о стиллере SAMPSTEAL от Karabawka & MishaN!\n{00FF00} - Желаешь перейти на сайт стиллера SAMPSTEAL после выхода из игры, чтобы узнать подробную информацию?", "Да", "Позже");
		}*/
		...
		/*if (g_bShowAd)
		{
			...
			addMessageToChatWindow("=========================[RakBot by MishaN]======================");
			memset_safe((void *)(g_dwSAMP_Addr + 0xB3960), 0xC3, 1);
			showSampDialog(0, 1488, 0, "RakBot.Ru", "{FFFFFF}Желаете заработать денег в интернете?\nВ этом Вам поможет уникальная программа под названием RakBot, с помощью которого вы сможете прокачивать аккаунты, зарабатывать виртуальные средства,\nкоторые Вы впоследствии можете продавать, как на рынке, так и в официальном магазине CM SHOP.\n\nДанная программа [RakBot] {FFFF00}работает независимо от наличия игры [GTA San Andreas] на компьютере{FFFFFF}.\nСледовательно, Вы сможете запускать ее на виртуальных компьютерах. Также бот позволяет запускать несколько аккаунтов одновременно.\n\nНиже перечислены способы заработка с помощью данной программы:\n\n{00FF00}- продажа аккаунтов-пустышек, прокачанных с помощью бота;\n- продажа игровой валюты, полученной по системе приглашений;\n- продажа игровой валюты, полученной с помощью ботов работ;\n- прокачивание аккаунтов на заказ;\n- другие способы, которые Вы можете придумать сами.\n\n{FFFFFF}Если Вас заинтересовал данный проект, можете посетить сайт программы: RakBot.Ru.\nНажмите 'ОК' для перехода после выхода из игры или 'Позже', если предпочтете посетить страницу позже.", "OK", "Позже");
		}*/
			...
			if (set.d3dtext_hud)
			{
				if (cheat_panic() || cheat_state->state == CHEAT_STATE_NONE)
				{
					...
					if (!set.flickering_problem)
					{
						if (iIsSAMPSupported && !game_inited)
						{
							...
							pD3DFont->PrintShadow(1, pPresentParam.BackBufferHeight - 21, D3DCOLOR_XRGB(255, 163, 0), "Спасибо Logotipo, iMaddy, Kukushka, Голубь, Mstyle. Оформление темы: Logan");
						}
						else
						{
				...
				else
				{
					...
					if (g_Players && g_Players->pLocalPlayer && g_Players->pLocalPlayer->iIsActive && set.show_infobar && !gta_menu_active())
					{
						...
						_snprintf_s(buffer, sizeof(buffer)-1, "Очки(LVL): %d Анимация: %d", g_Players->iLocalPlayerScore, g_Players->pLocalPlayer->sCurrentAnimID);
						pD3DFont->PrintShadow(pPresentParam.BackBufferWidth - 275.0f, pPresentParam.BackBufferHeight - 150.0f, D3DCOLOR_XRGB(255, 165, 0), "Ник(ID):");
